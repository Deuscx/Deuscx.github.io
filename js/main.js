 ...
 */
  ...
  const handleVisibilityChange= function(){
    if (GLOBAL_CONFIG.visibility.enable) {
      // title变化
      window.originTitle = document.title;
      let titleTime;
      document.addEventListener('visibilitychange', function () {
        if (document.hidden) {
          document.title = window.GLOBAL_CONFIG.visibility.hide_text;
          clearTimeout(titleTime);
        }
        else {
          document.title = window.GLOBAL_CONFIG.visibility.show_text + window.originTitle;
  ...
  function initDPlayer(){
    ...
    fetch( 'https://api.i-meto.com/meting/api?server=netease&type=playlist&id=35798529',
